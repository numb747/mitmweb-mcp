...
这条边界不只写在文档里,也声明给了客户端:九个读工具都带 `readOnlyHint: true`,
客户端可以据此自行决定不弹确认。`replay_flow` 标的是 `destructiveHint: true` ——
它对**你的**会话只是追加,但发上线的是当初抓到的那个请求,重放一条 `DELETE`
就真的会删掉东西。它本来就该是唯一一个需要停下来问一句的工具。
